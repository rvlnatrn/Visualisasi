...
if {"ym", "after_discount"}.issubset(df.columns):
    ...
    fig_trend = px.line(trend, x="ym", y="after_discount",
                        markers=True, title="Sales Trend by Month (after_discount)")
...
if {"category", "after_discount"}.issubset(df.columns):
    ...
    fig_cat = px.bar(cat_sales, x="after_discount", y="category",
                     orientation="h", title="Top Categories by Sales")
...
if {"payment_method", "after_discount"}.issubset(df.columns):
    ...
    fig_pay = px.pie(pay, names="payment_method", values="after_discount",
                     title="Sales by Payment Method", hole=0.35)
...
if {"sku_name", "after_discount"}.issubset(df.columns):
    ...
    fig_tp = px.bar(top_prod, x="after_discount", y="sku_name",
...
if {"customer_id", "after_discount"}.issubset(df.columns):
    top_cust = (df.groupby("customer_id", as_index=False)["after_discount"]
                  .sum().sort_values("after_discount", ascending=False).head(10))
    fig_tc = px.bar(top_cust, x="after_discount", y="customer_id",
